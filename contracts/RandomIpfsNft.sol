...
contract RandomIpfsNft is VRFConsumerBaseV2Plus, ERC721URIStorage, Ownable {
  ...
  // The owner of the contract can withdraw the ETH
   ...
   constructor(
        ...
        address vrfCoordinatorV2, uint64 subscriptionId, bytes32 gasLane, uint32 callbackGasLimit, string[3] memory dogTokenUris, uint256 mintFee
  ...
  function requestNft() public payable returns (uint256 requestId) {
    ...
    requestId = i_vrfCoordinator.requestRequestWords(
    i_subscriptionId,
    i_gasLane,
  ...
  function fulfillRandomWords(
    ...
    function getBredFromModdedRng(uint256 moddedRng) public pure returns(Breed) { //This is to get random number just as in Raffle smart contract 
      uint256 cumulativeSum = 0;
      uint256[3] memory chanceArray = getChanceArray();
      // modelling = 25
      // i = 0, for second iteration it will be 1
      // cumulativeSum = 0, for second iteration it will be 10.
      for(uint256 i = 0; i < chanceArray.length; i++){ // i = 0 will not be true, then it will jump to second line "cumulativeSum += chanceArray[i];"
        if(moddedRng >= cumulativeSum && moddedRng < cumulativeSum + changeArray[i]){ // For the first time, changeArray[i] is 10
          return Breed(i);
        }
  ...
  function getChanceArray() public returns(uint256[3] memory) { // The chanceArray() will represent different chances of different dogs.
