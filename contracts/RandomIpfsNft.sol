...
s_requestIdToSender[requestId] = msg.sender; // the requestId is set to msg.sender when the requestNft() is called.
  ...
  function fulfillRandomWords(
    ...
    uint256 moddedRng = randomWords[0] % MAX_CHANCE_VALUE; // moddedRng will be any value between 0 and 99
    ...
    function getBredFromModdedRng(uint256 moddedRng) public pure returns(Breed) { //This is to get random number just as in Raffle smart contract 
      ...
      for(uint256 i = 0; i < chanceArray.length; i++){
        if(moddedRng >= cumulative && moddedRng < cumulativeSum + changeArray[i]){
          return Breed(i);
        }
